...
CREATE TABLE ServicePackages (
    ...
    created_at DATETIME2 NOT NULL DEFAULT GETDATE(),
    updated_at DATETIME2 NOT NULL DEFAULT GETDATE(),
    CONSTRAINT UQ_ServicePackages_Code UNIQUE(package_code),
    CONSTRAINT CK_ServicePackages_Code CHECK (package_code IN ('BASIC', 'PREMIUM', 'DELUXE'))
);
GO
...
CREATE TABLE TimeSlots (
    ...
    is_active BIT NOT NULL DEFAULT 1,
    CONSTRAINT CK_TimeSlots_Capacity CHECK (capacity > 0),
    CONSTRAINT CK_TimeSlots_Time CHECK (start_time < end_time),
    CONSTRAINT UQ_TimeSlots UNIQUE (start_time, end_time)
);
GO
...
CREATE TABLE WalletTransactions (
    ...
    created_at DATETIME2 NOT NULL DEFAULT GETDATE(),
    CONSTRAINT FK_WalletTransactions_Wallets FOREIGN KEY (wallet_id) REFERENCES Wallets(wallet_id),
    CONSTRAINT FK_WalletTransactions_Bookings FOREIGN KEY (booking_id) REFERENCES Bookings(booking_id),
    CONSTRAINT CK_WalletTransactions_Type CHECK (transaction_type IN ('TOP_UP', 'PAYMENT', 'REFUND'))
);
GO
...
CREATE TABLE Payments (
    ...
    CONSTRAINT CK_Payments_Method CHECK (payment_method IN ('VNPAY', 'WALLET', 'CASH', 'QR')),
    CONSTRAINT CK_Payments_Status CHECK (payment_status IN ('SUCCESS', 'REFUNDED')),
...
CREATE TABLE LoyaltyProfiles (
    ...
    created_at DATETIME2 NOT NULL DEFAULT GETDATE(),
    updated_at DATETIME2 NOT NULL DEFAULT GETDATE(),
    CONSTRAINT UQ_LoyaltyProfiles_User UNIQUE(user_id),
    CONSTRAINT FK_LoyaltyProfiles_Users FOREIGN KEY (user_id) REFERENCES Users(user_id),
    CONSTRAINT CK_LoyaltyProfiles_Points CHECK (total_points >= 0),
    CONSTRAINT CK_LoyaltyProfiles_Tier CHECK (current_tier IN ('BRONZE', 'SILVER', 'GOLD', 'DIAMOND'))
);
GO
